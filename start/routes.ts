  ...
  .group(() => {
    router
      .group(() => {
        ...
        router.get('/sets', [SetsController, 'index']).use(middleware.auth())
        router.get('/sets/:id/details', [SetsController, 'details'])
        router.get('/sets/:id/cards', [SetsController, 'cards'])
      })
      .prefix('v1')
  })
  .prefix('api')
