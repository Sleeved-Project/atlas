...
test.group('CardMapper', (group) => {
  ...
  test('toCardsWithFolioOccurenceOutputDTO - should return correctly formatted DTO with paginated cards', ({
    assert,
  }) => {
    const cardFolio1 = { occurrence: 3 }
    const cardFolio2 = { occurrence: 1 }

    const card1 = new Card()
    card1.id = '1'
    card1.imageSmall = 'card1-small.jpg'
    card1.cardFolios = [cardFolio1] as HasMany<typeof CardFolio>

    const card2 = new Card()
    card2.id = '2'
    card2.imageSmall = 'card2-small.jpg'
    card2.cardFolios = [cardFolio2] as HasMany<typeof CardFolio>
    ...
    const mockPaginator = {
      ...
      all: sandbox.stub().returns([card1, card2]),
    }

    const result = CardMapper.toCardsWithFolioOccurenceOutputDTO(
      mockPaginator as unknown as ModelPaginatorContract<Card>
    )
    ...
    assert.equal(result.data[0].imageSmall, 'card1-small.jpg')
    assert.equal(result.data[0].occurrence, 3)

    assert.equal(result.data[1].id, '2')
    assert.equal(result.data[1].imageSmall, 'card2-small.jpg')
    assert.equal(result.data[1].occurrence, 1)

    sinon.assert.calledOnce(mockPaginator.getMeta)
    sinon.assert.calledOnce(mockPaginator.all)
  })

  test('toCardsWithFolioOccurenceOutputDTO - should return 0 occurrence when cardFolios is empty', ({
    assert,
  }) => {
    const card = new Card()
    card.id = '1'
    card.imageSmall = 'card-small.jpg'
    card.cardFolios = [] as unknown as HasMany<typeof CardFolio>
    ...
    assert.equal(result.data[0].occurrence, 0)
  })

  test('toCardsWithFolioOccurenceOutputDTO - should return empty data array when no cards', ({
    assert,
  }) => {
    const mockPaginator = {
      getMeta: sandbox.stub().returns({
  ...
  test('toCardsWithFolioOccurenceOutputDTO - should take first cardFolio occurrence when multiple exist', ({
    assert,
  }) => {
    const cardFolio1 = { occurrence: 5 }
    const cardFolio2 = { occurrence: 2 }

    const card = new Card()
    card.id = '1'
    card.imageSmall = 'card-small.jpg'
    card.cardFolios = [cardFolio1, cardFolio2] as HasMany<typeof CardFolio>
    ...
    assert.equal(result.data[0].occurrence, 5)
  })
})
